        ...
        <div className="pt-6 border-t border-white/5 space-y-6">
          ...
          <div className="pt-6 border-t border-white/5">
            ...
            <div className="space-y-3">
              ...
              {(opSettings.test_presets || []).map((preset: any, idx: number) => (
                <div key={idx} className="grid grid-cols-1 md:grid-cols-[2fr_1.5fr_1fr_1fr_auto] gap-2 bg-black/40 border border-white/10 rounded-[2px] p-2 items-center">
                  <input 
                    type="text" 
                    value={preset.name || ''} 
                    onChange={(e) => {
                      const newPresets = [...(opSettings.test_presets || [])];
                      newPresets[idx] = { ...newPresets[idx], name: e.target.value };
                      setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                    }}
                    onBlur={() => updateOpSetting('test_presets', opSettings.test_presets)}
                    placeholder="예: 퀴즈, 모의고사"
                    className="w-full bg-transparent border-b border-white/10 px-2 py-1.5 text-xs font-bold text-white outline-none focus:border-blue-500"
                  />
                  <select
                    value={preset.type || 'count'}
                    onChange={(e) => {
                      const newPresets = [...(opSettings.test_presets || [])];
                      newPresets[idx] = { ...newPresets[idx], type: e.target.value };
                      setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                      updateOpSetting('test_presets', newPresets);
                    }}
                    className="w-full bg-[#1a1a1a] border-b border-white/10 px-2 py-1.5 text-xs font-bold text-gray-300 outline-none focus:border-blue-500"
                  >
                    <option value="count">개수형 (맞은 개수)</option>
                    <option value="score">백분율형 (100점 만점)</option>
                  </select>
                  <input 
                    type="number" 
                    value={preset.max || ''} 
                    onChange={(e) => {
                      const newPresets = [...(opSettings.test_presets || [])];
                      newPresets[idx] = { ...newPresets[idx], max: parseInt(e.target.value) || 0 };
                      setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                    }}
                    onBlur={() => updateOpSetting('test_presets', opSettings.test_presets)}
                    placeholder="만점"
                    className="w-full bg-transparent border-b border-white/10 px-2 py-1.5 text-xs font-bold text-emerald-400 outline-none focus:border-emerald-500"
                  />
                  <input 
                    type="number" 
                    value={preset.default_cut || ''} 
                    onChange={(e) => {
                      const newPresets = [...(opSettings.test_presets || [])];
                      newPresets[idx] = { ...newPresets[idx], default_cut: parseInt(e.target.value) || 0 };
                      setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                    }}
                    onBlur={() => updateOpSetting('test_presets', opSettings.test_presets)}
                    placeholder="커트(선택)"
                    className="w-full bg-transparent border-b border-white/10 px-2 py-1.5 text-xs font-bold text-amber-400 outline-none focus:border-amber-500"
                  />
                  <button 
                    onClick={() => {
                      const newPresets = (opSettings.test_presets || []).filter((_: any, i: number) => i !== idx);
                      setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                      updateOpSetting('test_presets', newPresets);
                    }}
              ...
              <button 
                onClick={() => {
                  const newPresets = [...(opSettings.test_presets || []), { name: '', type: 'count', max: 10, default_cut: 8 }];
                  setOpSettings((prev:any) => ({ ...prev, test_presets: newPresets }));
                }}
                className="w-full py-2 bg-white/5 hover:bg-white/10 border border-white/10 border-dashed rounded-[2px] text-[10px] font-black text-gray-400 hover:text-white transition-all uppercase tracking-widest"
              >
                + 새 테스트 템플릿 추가
              </button>
            </div>
