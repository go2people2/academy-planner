...
export default function StudentExamSubmission({ 
  ...
  const [backupAnswers, setBackupAnswers] = useState<Record<string, number | string | number[]> | null>(null); // 💡 초기화 전 복원용 백업 상태
  const [submitting, setSubmitting] = useState(false);
  const [loadingExams, setLoadingExams] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  // 💡 학생 학년을 기본 필터로 설정 (없으면 전체)
  const [selectedGrade, setSelectedGrade] = useState(studentGrade || 'All');

  // 💡 AI OMR 사진 스캔을 위한 상태 및 Ref
  const [isScanning, setIsScanning] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const directFileInputRef = useRef<HTMLInputElement>(null); // 💡 시험지 미배정 시 다이렉트 파일 인풋
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = ''; // 동일 파일 재업로드 가능하게 리셋
  };
  ...
  const [incorrectResolved, setIncorrectResolved] = useState<Record<string, boolean>>({}); // 💡 오답 정리 완료 여부 상태
  const [examStats, setExamStats] = useState<{ // 💡 시험 통계 상태 추가
    average: number;
    min: number;
    max: number;
    stdDev: number;
  ...
  // 💡 submission이 세팅될 때 로컬스토리지에서 오답 정리 상태 복원
  useEffect(() => {
    if (submission) {
      try {
  ...
  const handleClearAnswers = () => {
    ...
    if (confirm('작성 중인 OMR 마킹 정보가 초기화됩니다. 계속하시겠습니까?')) {
      setBackupAnswers(answers); // 현재 상태 백업
      setAnswers({});
    }
  };
  ...
  const handleRestoreAnswers = () => {
    if (backupAnswers) {
      ...
      setBackupAnswers(null); // 사용 완료 후 리셋
    }
  };
  ...
  const fetchExams = async () => {
    ...
    // 💡 정규식 매핑 개선: "- #6178 : ," 같은 복합 일지 포맷에서 "#6178" 시험지 코드만 안전하게 파싱 추출
    ...
    try {
      // 1단계: 시험지 코드(exam_code)로 정확히 일치하는 것 찾기
      ...
      // 2단계: 없으면, UUID 형식인지 체크하여 ID로 매칭 시도
      ...
      // 3단계: 그것도 없으면, 시험지 제목(title) 부분 일치 검색 (ilike)
      if (!data) {
      ...
      if (data) {
        setExams([data]);
        setSelectedExam(data); // 💡 자동 선택하여 OMR 창 또는 결과 창으로 바로 진입
      } else {
        setExams([]);
      }
    } catch (err) {
  ...
  const checkExistingSubmission = async (examId: string) => {
  ...
  // 실시간 변경 시 재조회 (선생님이 정답 공개로 토글했는지 체크용)
  const handleRefreshStatus = async () => {
    if (!selectedExam) return;
    await checkExistingSubmission(selectedExam.id);
  };

  // 💡 제출 이력 항목 클릭 시 결과 뷰로 강제 바인딩
  ...
  const handleMark = (qNum: number, choice: number, isMulti: boolean) => {
    if (submission) return; // 이미 제출한 경우 마킹 불가
    const qKey = qNum.toString();
  ...
  const handleSubmitAnswers = async () => {
    ...
    setSubmitting(true);
    try {
      // 💡 실시간 자동 채점 및 채점 보고서 생성
      let correctCount = 0;
      let gradableCount = 0; // 채점 가능한 문항 수 (서술형 제외)
      ...
      const payload = {
        ...
        total_score: autoScore, // 일단 서술형 채점 전엔 자동채점 점수와 동일
        wrong_questions: wrongQuestions,
        submitted_at: new Date().toISOString(),
        reveal_answers: false // 💡 기본은 비공개 상태
      };
