...
const TIMER_THEMES = [
  { text: 'text-indigo-400', bg: 'bg-indigo-600', border: 'border-indigo-500', ring: 'ring-indigo-500/30', glow: 'shadow-indigo-500/20' },
  { text: 'text-emerald-400', bg: 'bg-emerald-600', border: 'border-emerald-500', ring: 'ring-emerald-500/30', glow: 'shadow-emerald-500/20' },
  { text: 'text-rose-400', bg: 'bg-rose-600', border: 'border-rose-500', ring: 'ring-rose-500/30', glow: 'shadow-rose-500/20' },
  { text: 'text-cyan-400', bg: 'bg-cyan-600', border: 'border-cyan-500', ring: 'ring-cyan-500/30', glow: 'shadow-cyan-500/20' },
  { text: 'text-amber-400', bg: 'bg-amber-600', border: 'border-amber-500', ring: 'ring-amber-500/30', glow: 'shadow-amber-500/20' },
  { text: 'text-purple-400', bg: 'bg-purple-600', border: 'border-purple-500', ring: 'ring-purple-500/30', glow: 'shadow-purple-500/20' },
];

const getTimerTheme = (index: number) => TIMER_THEMES[index % TIMER_THEMES.length];

import { useClassroomModeState } from './hooks/useClassroomModeState';
...
export default function ClassroomMode({ students, onSave, onClose, selectedDate, academyInfo, selectedTeacherId, isLight = false }: ClassroomModeProps) {
  ...
  const handleFinishHour = async (hour: number) => {
    ...
    if (confirm(confirmMsg)) {
      for (const s of studentsToMark) {
        await localSave(s, { attendance_status: ATTENDANCE_STATUS.ABSENT });
      }
  ...
  const handleResetHour = async (hour: number) => {
    ...
    if (confirm(confirmMsg)) { for (const s of studentsToReset) { await localSave(s, { attendance_status: ATTENDANCE_STATUS.BEFORE }); } }
  };
  ...
  const allTodayStudents = useMemo(() => {
    ...
    Array.from(uniqueOriginalStudents.values()).forEach(s => {
      ...
      if (session) {
        ...
        if (sDate && sDate !== selectedDate) return;
      }
      
      const status = session?.attendance_status || ATTENDANCE_STATUS.BEFORE;
      if (status.startsWith(ATTENDANCE_STATUS.EXCLUDED)) return;
      ...
      const isLogMakeup = (log: any) => log?.is_pure_makeup === true;
      const regularSession = todayLogs.find((l: any) => (l.course_name === '정규' || !l.course_name) && !isLogMakeup(l));
      const rawMakeupLogs = todayLogs.filter((l: any) => isLogMakeup(l));
      ...
      makeupLogs.forEach((mLog: any) => {
        ...
        const isPureMakeup = isLogMakeup(mLog);
      ...
      activeElectives.forEach((c: any, cIdx: number) => {
        ...
        const electiveLog = todayLogs.find((l: any) => l.course_name === courseSubject && !isLogMakeup(l));
      ...
      if (!hasRegularSession && activeElectives.length === 0 && makeupLogs.length === 0 && (status.startsWith(ATTENDANCE_STATUS.SUPPLEMENT) || session)) {
        const isMakeupSession = session ? isLogMakeup(session) : false;
        expandedResult.push({
          ...s,
          courseName: session?.course_name || '정규',
          __courseType: isMakeupSession ? 'makeup' : 'regular',
          todaySession: session
        });
      }
    });

    return expandedResult.filter(s => {
      if (selectedTeacherId && selectedTeacherId !== 'All' && s.teacher_id !== selectedTeacherId) return false;
      const studentHour = getStudentHour(s);
      
      // 스케줄이 아예 없는 학생은 라이브 방에 들어가지 않습니다.
      if (studentHour === 999) return false;
  ...
  const handleCardClick = async (student: any) => {
    ...
    const isPendingMakeup = status.startsWith(ATTENDANCE_STATUS.SUPPLEMENT) || student.__courseType === 'makeup' || (student.todaySession && student.todaySession.moved_to_hour !== undefined && student.todaySession.moved_to_hour !== null);
    if (status === ATTENDANCE_STATUS.BEFORE || (isPendingMakeup && !status.startsWith(ATTENDANCE_STATUS.PRESENT))) {
    ...
    if (activeStudent?.id === student.id && activeStudent?.__courseType === student.__courseType) { 
    ...
    const isOriginalScheduledHour = !isMakeup && (() => {
      ...
      if (student.__courseType === 'elective' || student.isSpecialClass) {
        ...
        try {
          ...
          const currentSubject = student.courseName || student.electiveCourse?.subject || student.todaySession?.course_name;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar-v pr-2 pb-20 px-4">
        <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9 gap-3">
          {allTodayStudents.map((s, idx) => {
            ...
            const isActive = activeStudent?.id === s.id && activeStudent?.__courseType === s.__courseType;

            return (
              <React.Fragment key={`${s.id}-${s.__courseType || 'regular'}`}>
                    ...
                    >
                      {activeTab === 'timer' ? (
                        ...
                        <>
                          ...
                          <div className="text-center px-1"><h3 className={`text-2xl font-bold tracking-tight leading-none ${isPureAttend ? (isLight ? 'text-gray-500 font-semibold' : 'text-gray-500') : (isLight ? 'text-[#0f172a]' : 'text-white')}`}>{s.name}</h3><div className="mt-2 flex flex-col items-center gap-1">
                            {(() => { 
                              ...
                              return (
                                <div className="flex flex-row items-center gap-1 justify-center">
                                  ...
                                  {(() => {
                                    const cName = s.courseName || s.todaySession?.course_name;
                                    const courseLabel = s.__courseType === 'elective' 
                                      ? (s.__courseSubject || '특강')
                                      : (s.__courseType === 'makeup' 
                                        ? (cName && cName !== '정규' ? `${cName}` : '정규보강')
                                        : '정규');
                                    const isSpecial = s.__courseType === 'elective' || (s.__courseType === 'makeup' && s.todaySession?.course_name && s.todaySession.course_name !== '정규');
                                    return (
                                      <p className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${isPureAttend && !isLight ? 'text-gray-600 border-white/5 bg-white/5 opacity-50' : (isSpecial ? (isLight ? 'text-purple-800 border-purple-200 bg-purple-50' : 'text-fuchsia-400 border-fuchsia-500/30 bg-fuchsia-500/15') : (isLight ? 'text-gray-700 border-gray-200 bg-gray-50' : 'text-gray-400 border-white/10 bg-white/5'))}`}>{courseLabel}</p>
                                    );
                                  })()}
                                </div>
                              ); 
                          ...
                          {(() => {
                            const isMakeupCard = s.__courseType === 'makeup' || s.todaySession?.is_pure_makeup === true;
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 px-8 py-3 bg-[#0a0a0a]/95 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center gap-6 z-30 shadow-2xl shrink-0">
        <div className="flex items-center gap-2"><span className="text-[9px] font-black text-gray-500 uppercase tracking-tighter">전체</span><span className="text-xl font-black text-white">{stats.total}</span></div>
