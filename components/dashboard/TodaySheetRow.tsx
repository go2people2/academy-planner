    ...
    <>
      ...
      <tr style={{ display: 'none' }}>
        <td colSpan={activeColumns.length}>
          <AnimatePresence>
            {isCwEditorOpen && <HomeworkEditor title="오늘 할 일 교재 입력" student={student} homeworkJson={formData.classwork_json || []} masterTextbooks={masterTextbooks} academyInfo={academyInfo} onUpdate={(newJson) => { const update = { classwork_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.classwork_json || [], 'classwork'); setIsCwEditorOpen(false); }} isLight={false} />}
            {isCcwEditorOpen && <HomeworkEditor title="수행 진도 교재 입력" student={student} homeworkJson={formData.completed_classwork_json || []} masterTextbooks={masterTextbooks} academyInfo={academyInfo} onUpdate={(newJson) => { const update = { completed_classwork_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.completed_classwork_json || [], 'completed_classwork'); setIsCcwEditorOpen(false); }} isLight={false} />}
            {isHwEditorOpen && <HomeworkEditor title="오늘 숙제 교재 입력" student={student} homeworkJson={formData.homework_json || []} masterTextbooks={masterTextbooks} academyInfo={academyInfo} onUpdate={(newJson) => { const update = { homework_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.homework_json || [], 'homework'); setIsHwEditorOpen(false); }} isLight={false} />}
