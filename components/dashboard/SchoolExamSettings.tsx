...
export default function SchoolExamSettings({ academyInfo, students, onUpdateAcademyInfo }: SchoolExamSettingsProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="bg-white/5 border border-white/10 rounded-[6px] overflow-hidden shadow-2xl p-4 space-y-4">
        {/* 1행: 시험 기간 선택 */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-[2px] px-2.5 py-1">
              <Calendar size={15} className="text-rose-500 opacity-70" />
              <input
                type="number"
                value={selectedYear}
                onChange={e => setSelectedYear(e.target.value)}
                className="w-16 bg-transparent text-[13px] font-black text-white outline-none text-center"
              />
            </div>
            ...
            <div className="flex items-center gap-1.5 flex-nowrap">
              {EXAM_TYPES.map(type => (
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] font-black rounded-[2px] border whitespace-nowrap transition-all ${
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <span className="text-[11px] font-bold text-gray-400">
              현재 활성: <span className="text-rose-400 font-black">{currentActiveLabel}</span>
            </span>
            <button
              onClick={handleSetCurrentPeriod}
              className={`px-4 py-1.5 rounded-[2px] text-[11px] font-black uppercase tracking-wider transition-all shrink-0 ${
                currentPeriod === `${selectedYear}-${selectedType}`
                  ? 'bg-rose-500/10 text-rose-400 border border-rose-500/30'
                  : 'bg-rose-600 text-white hover:bg-rose-500 shadow-md active:scale-95'
              }`}
            >
        ...
        <div className="border-t border-white/10 pt-3.5 space-y-2.5">
          <p className="text-[11px] font-black text-gray-400 tracking-wider">학교별 시험 일정 등록</p>
          <div className="flex flex-wrap items-center gap-2.5">
            <input
              ...
              className="flex-1 min-w-[150px] bg-black/40 border border-white/10 rounded-[2px] px-3 py-1.5 text-[12px] font-bold text-white outline-none focus:border-rose-500"
            />
            <input
              type="text"
              ...
              className="flex-1 min-w-[180px] bg-black/40 border border-white/10 rounded-[2px] px-3 py-1.5 text-[12px] font-bold text-white outline-none focus:border-rose-500"
            />
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-32 bg-black/40 border border-white/10 rounded-[2px] px-2.5 py-1.5 text-[12px] text-white focus:outline-none focus:border-rose-500 [color-scheme:dark]"
              />
            ...
            <button
              ...
              disabled={isSaving}
              className="px-5 py-1.5 bg-rose-600 hover:bg-rose-500 text-white text-[11px] font-black rounded-[2px] uppercase tracking-wider transition-all shadow-md active:scale-95 flex items-center gap-1.5 shrink-0"
            >
            ...
            <div className="pt-2 flex flex-wrap gap-1.5 items-center">
              ...
              {HIGH_SUBJECTS.map(sub => (
                <button
                  ...
                  className={`px-2 py-0.5 text-[10px] font-black rounded-[2px] border transition-all ${
                    newExam.subject === sub
              ...
              <input
                ...
                className="ml-2 w-16 bg-transparent border-b border-white/10 px-1 py-0.5 text-[10px] font-bold text-white outline-none focus:border-rose-500"
              />
            </div>
          )}
        </div>
              ...
              return (
                <div key={exam.id} className="flex flex-wrap items-center gap-2 px-4 py-3 bg-white/[0.08] transition-all">
                  <input
                    ...
                    className="w-24 bg-black/40 border border-white/20 rounded-[2px] px-2 py-1 text-[12px] text-white focus:outline-none focus:border-blue-500"
                    placeholder="학교명"
                  />
                  <input
                    type="text"
                    ...
                    className="w-28 bg-black/40 border border-white/20 rounded-[2px] px-2 py-1 text-[12px] text-white text-center focus:outline-none focus:border-blue-500"
                    placeholder="예: 1,2 / 3"
                  ...
                  <input
                    ...
                    className="w-16 bg-black/40 border border-white/20 rounded-[2px] px-2 py-1 text-[12px] text-white focus:outline-none focus:border-blue-500"
                    placeholder="과목"
                  />
                  <input
                    type="date"
                    ...
                    className="w-28 bg-black/40 border border-white/20 rounded-[2px] px-2 py-1 text-[12px] text-white focus:outline-none focus:border-blue-500 [color-scheme:dark]"
                  />
            ...
            return (
              <div key={exam.id} className="flex items-center justify-between px-4 py-2 group hover:bg-white/[0.04] transition-all">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  {exam.grade && <span className="px-2 py-0.5 rounded bg-white/10 text-[11px] font-bold text-rose-300 whitespace-nowrap">{formatGradeDisplay(exam.grade)}</span>}
