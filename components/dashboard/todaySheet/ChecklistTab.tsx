        ...
        <table className="w-full border-collapse table-fixed">
          <thead>
            ...
            <tr className="border-b border-white/5 bg-[#121212] text-gray-400 uppercase tracking-widest text-[9.5px] font-black">
              <th rowSpan={2} style={{ width: colWidths.name, minWidth: colWidths.name }} className="py-3 px-3 border-r border-white/5 text-left sticky left-0 bg-[#121212] z-30 shadow-[2px_0_5px_rgba(0,0,0,0.2)]">
                학생 이름
              </th>
              {topics.map(t => (
                <th key={t.id} colSpan={2} className="py-2.5 px-3 border-r border-white/5 text-center group relative min-w-[120px]">
                  <div className="flex items-center justify-center gap-1.5">
                    <span className="text-gray-100 text-[11px] font-black truncate max-w-[80px]" title={t.title}>
                      {t.title}
                    </span>
                    <button 
            ...
            <tr className="border-b border-white/5 bg-[#121212] text-gray-500 uppercase tracking-widest text-[8.5px] font-black">
              {topics.map(t => (
                <React.Fragment key={`sub-${t.id}`}>
                  <th style={{ width: colWidths.check, minWidth: colWidths.check }} className="py-1.5 px-2 border-r border-white/5 text-center">완료</th>
                  <th style={{ width: colWidths.memo, minWidth: colWidths.memo }} className="py-1.5 px-2 border-r border-white/5 text-left">메모 / 특이사항</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
              ...
              students.map((student, idx) => {
                ...
                return (
                  <tr key={student.id} className={`${rowBg} border-b border-white/5 hover:bg-[#1a1a1a] transition-colors align-middle text-[11px]`}>
                    {/* 1열 고정 학생명 */}
                    <td className="py-2.5 px-3 border-r border-white/5 font-black text-gray-200 sticky left-0 bg-inherit z-20 shadow-[2px_0_5px_rgba(0,0,0,0.2)]">
                      <div className="flex flex-col gap-0.5">
                        <span className="truncate max-w-[120px]">{student.name}</span>
                        <span className="text-[9px] text-gray-500 font-bold tracking-tight truncate max-w-[120px]">
