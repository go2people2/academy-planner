              ...
              if (idx === 0) {
                return (
                  <td 
                    ...
                    className={`py-3.5 px-3 ${borderClass} text-left ${isLight ? 'bg-[#f0f4f8]' : 'bg-[#050505]'}`}
                  >
                    <div className="flex flex-col gap-1 pr-1 select-none">
                      ...
                      {student.assigned_books && student.assigned_books.length > 0 ? (
                        <div className="flex flex-col gap-1.5 max-h-[140px] overflow-y-auto custom-scrollbar-v pr-0.5">
                          {student.assigned_books.map((book: string, bIdx: number) => {
                            ...
                            return (
                              <div 
                                key={bIdx} 
                                className={`px-2.5 py-1.5 ${isLight ? 'bg-[#f8f8f7] border-gray-200/85 text-[#37352f]' : 'bg-white/10 border-white/20 text-gray-100'} border rounded-[4px] text-[10.5px] font-extrabold truncate leading-tight shadow-sm`}
                                title={translated}
                              >
                                {translated}
                              </div>
                        ...
                        <div className={`text-[9.5px] ${isLight ? 'text-gray-400' : 'text-gray-600'} italic px-1`}>배정 교재 없음</div>
                      )}
...
const ManagementLogsTimeline = React.memo(function ManagementLogsTimeline({ student, isLight = false }: { student: Student, isLight?: boolean }) {
  ...
  return (
    <div className="mt-4">
      <div className={`text-[9px] font-black ${isLight ? 'text-amber-700' : 'text-amber-500/80'} tracking-wider uppercase mb-1.5 flex items-center gap-1`}>
        ⚠️ 관리 주의점 히스토리
      </div>
      {isLoading ? (
        <div className={`text-[9px] ${isLight ? 'text-gray-400' : 'text-gray-600'} italic px-1`}>이력 로딩 중...</div>
      ) : logs.length > 0 ? (
        <div className="flex flex-col gap-2 max-h-[160px] overflow-y-auto custom-scrollbar-v pr-0.5 select-text">
          {logs.map((log: any) => (
            <div 
              key={log.id} 
              className={`px-2 py-1.5 ${cardBg} border rounded-[4px] text-[10px] leading-relaxed`}
            >
              <div className={`flex justify-between text-[8px] ${headerColor} font-bold uppercase tracking-tighter mb-0.5 select-none`}>
                <span>{log.ams_teachers?.name || '시스템'}</span>
                <span>{log.created_at.slice(0, 10).replace(/-/g, '.')}</span>
              </div>
              <p className={`whitespace-pre-wrap break-all ${bodyColor} font-medium`}>{log.notes}</p>
            </div>
          ))}
        </div>
      ) : (
        <div className={`text-[9.5px] ${isLight ? 'text-gray-400' : 'text-gray-600'} italic px-1`}>등록된 주의점 이력 없음</div>
      )}
    </div>
  );
});
