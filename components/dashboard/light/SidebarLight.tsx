    ...
    <aside className="w-52 border-r border-[#edece9] bg-[#f7f7f5] flex flex-col p-3 sticky top-0 h-screen z-30">
      ...
      <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar-v">
        <nav className="space-y-1">
          <h3 className="text-[9px] font-bold text-[#37352f]/50 uppercase tracking-widest mb-2 px-2">Menu</h3>
          {menuOrder.map(id => {
            const menuMap: Record<string, any> = {
              live: <SidebarLink key="live" id="live" icon={<Zap size={14} className={isClassroomModeOpen ? "text-amber-500 fill-current animate-pulse" : "text-amber-500"} />} label="수업 시작 (LIVE)" active={isClassroomModeOpen} onClick={() => onStartClass()} variant="blue" isDragging={draggedId === 'live'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
              ...
              progress: <SidebarLink key="progress" id="progress" icon={<Activity size={14} className="text-teal-500" />} label="교재별진도" active={viewMode === 'progress'} onClick={() => { setViewMode('progress'); setSelectedFilter('All'); }} isDragging={draggedId === 'progress'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
              exams: <SidebarLink key="exams" id="exams" icon={<FileText size={14} className="text-blue-500" />} label="기출문제 관리" active={viewMode === 'exams'} onClick={() => { setViewMode('exams'); setSelectedFilter('All'); }} isDragging={draggedId === 'exams'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
              wrongAnswersAdmin: <SidebarLink key="wrongAnswersAdmin" id="wrongAnswersAdmin" icon={<BookOpen size={14} className="text-emerald-500" />} label="오답노트 관리" active={viewMode === 'wrongAnswersAdmin'} onClick={() => { setViewMode('wrongAnswersAdmin'); setSelectedFilter('All'); }} isDragging={draggedId === 'wrongAnswersAdmin'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
              studentEdit: <SidebarLink key="studentEdit" id="studentEdit" icon={<UserCog size={14} className="text-amber-600" />} label="학생정보수정" active={viewMode === 'studentEdit'} onClick={() => { setViewMode('studentEdit'); setSelectedFilter('All'); setSelectedDays([]); }} isDragging={draggedId === 'studentEdit'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
              monthlyChanges: <SidebarLink key="monthlyChanges" id="monthlyChanges" icon={<ArrowLeftRight size={14} className="text-indigo-500" />} label="이번 달 변동 사항" active={viewMode === 'monthlyChanges'} onClick={() => { setViewMode('monthlyChanges'); setSelectedFilter('All'); }} isDragging={draggedId === 'monthlyChanges'} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd} />,
            };
              ...
              <div className="pt-1">
                <div className="relative group">
                  <select value={selectedTeacherId} onChange={(e) => setSelectedTeacherId(e.target.value)} className="w-full bg-white border border-[#edece9] rounded-[2px] py-2 px-3 text-[10px] font-black text-[#37352f] outline-none appearance-none cursor-pointer hover:bg-[#edece9]/30 transition-all">
                    ...
                    {activeTeachers.map((t, idx) => <option key={t.id || idx} value={t.id} className="bg-white text-[#37352f]">{t.name} 선생님</option>)}
                  </select>
  ...
  return (
    <div
      ...
      className={`relative flex items-center gap-2 px-3 py-2 rounded-[2px] cursor-pointer transition-all group ${
        isDragging
          ? 'opacity-40 scale-95 border border-dashed border-gray-300'
          : active
    ...
    >
      <span className="opacity-0 group-hover:opacity-30 transition-opacity cursor-grab active:cursor-grabbing text-gray-400 shrink-0 -ml-1">
