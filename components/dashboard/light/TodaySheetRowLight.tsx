...
import { TodaySheetCell } from './TodaySheetCellLight';
...
useEffect(() => {
  ...
  return (
    <>
      <tr className={`group/row transition-all duration-300 border-b border-[#edece9] ${
        isSelected 
          ? 'bg-blue-600/10 hover:bg-blue-600/20' 
          : (rowIndex !== undefined && rowIndex % 2 !== 0)
            ? 'bg-[#f8f8f7] hover:bg-[#efeeee]' 
      ...
      <HistoryRows student={student} activeColumns={activeColumns} colWidths={colWidths} isExpanded={isHistoryExpanded} selectedDate={selectedDate} limit={props.historyLimit || 3} masterTextbooks={masterTextbooks} isLight={true} onUpdateStudentInfo={props.onUpdateStudentInfo} onSave={props.onSave} academyInfo={props.academyInfo} />

      {/* Editors Container (Invisible row) */}
      <tr style={{ display: 'none' }}>
        <td colSpan={activeColumns.length}>
          <AnimatePresence>
            {isCwEditorOpen && <HomeworkEditor title="오늘 할 일 교재 입력" student={student} homeworkJson={formData.classwork_json || []} masterTextbooks={masterTextbooks} onUpdate={(newJson) => { const update = { classwork_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.classwork_json || [], 'classwork'); setIsCwEditorOpen(false); }} isLight={true} />}
            {isCcwEditorOpen && <HomeworkEditor title="수행 진도 교재 입력" student={student} homeworkJson={formData.completed_classwork_json || []} masterTextbooks={masterTextbooks} onUpdate={(newJson) => { const update = { completed_classwork_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.completed_classwork_json || [], 'completed_classwork'); setIsCcwEditorOpen(false); }} isLight={true} />}
            {isHwEditorOpen && <HomeworkEditor title="오늘 숙제 교재 입력" student={student} homeworkJson={formData.homework_json || []} masterTextbooks={masterTextbooks} onUpdate={(newJson) => { const update = { homework_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.homework_json || [], 'homework'); setIsHwEditorOpen(false); }} isLight={true} />}
            {isNqEditorOpen && <HomeworkEditor title="다음 테스트 교재 입력" student={student} homeworkJson={formData.next_quiz_json || []} masterTextbooks={masterTextbooks} onUpdate={(newJson) => { const update = { next_quiz_json: newJson }; states.setFormData((prev: any) => ({ ...prev, ...update })); handleSave(update); }} onToggleKeepBook={handleToggleKeepBook} onClose={(finalJson) => { syncTextFromData(finalJson || formData.next_quiz_json || [], 'next_quiz'); setIsNqEditorOpen(false); }} isLight={true} />}
