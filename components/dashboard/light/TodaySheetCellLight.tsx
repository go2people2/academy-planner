                  ...
                  if (student.isSpecialClass) {
                    ...
                    return (
                      <span className="text-[13px] font-medium text-[#37352f] truncate transition-colors">
                        <span className="text-amber-800">{prefix}</span>
        ...
        {colId === 'attendance' && (() => {
          ...
          return (
            <div 
              ...
              className={`absolute inset-0 w-full h-full flex items-center justify-between px-3 text-[11px] cursor-pointer select-none transition-colors hover:bg-[#edece9]/30 z-30 ${
              isSupplementText ? 'text-blue-600 font-semibold' :
          ...
          <div className="relative w-full h-full flex items-start justify-between bg-blue-50/40 border-l-2 border-l-blue-300 py-1 px-2 gap-2">
            ...
            >
              {student.lastSession?.homework_text ? (
                <div className="text-[12px] font-medium text-blue-950 leading-[1.15] italic whitespace-pre-wrap break-all">
                  {student.lastSession.homework_text.split(/\n\s*\n/).map((para: string, i: number, arr: string[]) => (
                    <span key={i} className={`block ${i !== arr.length - 1 ? 'mb-1.5' : ''}`}>
                      ...
                      {para.split(/(\([월화수목금토일]\))/g).map((part, j) => 
                        part.match(/^\([월화수목금토일]\)$/) ? <span key={j} className="bg-amber-200/90 text-amber-950 font-black not-italic px-1 py-0.2 rounded border border-amber-400/50 shadow-sm inline-block mx-0.5">{part}</span> : part
                      )}
          ...
          <div className="w-full h-full p-1.5 flex flex-col gap-1 overflow-y-auto custom-scrollbar-v text-left">
            {(() => {
              ...
              if (assignedBooks.length === 0 && Object.keys(progressMap).length === 0) {
                return <span className="text-[10px] text-gray-400 italic select-none">-</span>;
              }
                  ...
                  <div key={bIdx} 
                    ...
                    {valParts.length > 1 ? (
                      <div className="flex flex-col gap-0.5 min-w-0 flex-1 py-0.5">
                        <div className="flex items-center gap-1.5 truncate">
                          <span className={`text-[8.5px] font-black px-1 rounded shrink-0 ${
                            isElectiveBook ? 'bg-amber-500 text-white' : targetTag === '공통' ? 'bg-blue-600 text-white' : 'bg-emerald-600 text-white'
                          }`}>
                            {targetTag.replace('선택:', '')}
                          </span>
                          <span className={`font-extrabold shrink-0 ${isElectiveBook ? 'text-amber-800' : 'text-emerald-800'}`}>
                            {bookTitle}
                          </span>
                          {val && daysElapsed >= 14 && (
                        ...
                        <div className="flex flex-col gap-0.5 pl-0.5">
                          {valParts.map((part, pIdx) => (
                            <div key={pIdx} className="text-[9.5px] text-gray-800 font-medium truncate flex items-center gap-1">
                              <span className="text-[8.5px] text-amber-600 font-bold shrink-0">{pIdx === 0 ? '①' : '②'}</span>
                      ...
                      <div className="flex items-center gap-1.5 truncate min-w-0 flex-1">
                        ...
                        <span className={`truncate text-[9.5px] ${val ? 'text-gray-800 font-medium opacity-90' : 'text-gray-500 italic'}`}>
                          {val || ''}
                        </span>
                        {val && daysElapsed >= 14 && (
                          <span className="text-[8px] font-black px-1 rounded bg-rose-600 text-white shrink-0">14일+ 정체</span>
                    ...
                    {onUpdateStudentInfo && (
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button
                          ...
                          className="p-0.5 rounded text-amber-600 hover:text-amber-700 hover:bg-amber-100 transition-colors"
                        >
                          <RefreshCw size={11} />
                        </button>
