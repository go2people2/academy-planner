...
import { buildMakeupPayload } from '../TeacherTasks';
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-[#f4f4f5] p-6 space-y-6">
      ...
      <div className="flex items-center justify-between shrink-0 bg-white border border-[#e3e2e0] rounded-xl p-4 shadow-sm">
        ...
        <div className="flex bg-[#f7f7f5] border border-[#e3e2e0] p-0.5 rounded-lg flex-wrap gap-0.5">
          <button 
            onClick={() => setActiveTab('makeups')}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all ${activeTab === 'makeups' ? 'bg-blue-600 text-white shadow-sm' : 'text-[#37352f]/50 hover:text-[#37352f]'}`}
          >
            <CalendarRange size={14} /> 보강 관리
          </button>
          <button 
            onClick={() => setActiveTab('tasks')}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all ${activeTab === 'tasks' ? 'bg-blue-600 text-white shadow-sm' : 'text-[#37352f]/50 hover:text-[#37352f]'}`}
          >
            <Sparkles size={14} /> 업무 목록
          </button>
          <button 
            onClick={() => setActiveTab('suggestions')}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all ${activeTab === 'suggestions' ? 'bg-blue-600 text-white shadow-sm' : 'text-[#37352f]/50 hover:text-[#37352f]'}`}
          >
            <MessageSquare size={14} /> 학생 건의
          </button>
          <button 
            onClick={() => setActiveTab('surveys')}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all ${activeTab === 'surveys' ? 'bg-blue-600 text-white shadow-sm' : 'text-[#37352f]/50 hover:text-[#37352f]'}`}
          >
            <Users size={14} /> 설문/수요조사
          </button>
          <button 
            onClick={() => setActiveTab('links')}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all ${activeTab === 'links' ? 'bg-blue-600 text-white shadow-sm' : 'text-[#37352f]/50 hover:text-[#37352f]'}`}
          >
        ...
        <AnimatePresence mode="wait">
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-2.5">
                          ...
                          <p className={`text-xs leading-relaxed ${task.is_completed ? 'text-gray-400 line-through' : 'text-[#37352f]'}`}>{task.content || '세부 설명이 없습니다.'}</p>
                        </div>
          ...
          {activeTab === 'makeups' && (
            <motion.div 
              ...
              className="absolute inset-0 flex flex-col space-y-4 overflow-hidden"
            >
              <div className="flex flex-wrap items-center justify-between gap-3 shrink-0 bg-[#f4f3ef] p-2.5 rounded-xl border border-[#e2e1dc]">
                <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
                  ...
                  <div className="relative w-48 shrink-0">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-[#d8d6d0] rounded-lg pl-8 pr-2.5 py-1.5 text-xs text-[#37352f] placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-all shadow-sm"
                    />
                  </div>
                  ...
                  <select
                    ...
                    className="bg-white border border-[#d8d6d0] rounded-lg px-2.5 py-1.5 text-xs text-[#37352f] font-bold focus:outline-none focus:border-blue-500 transition-all cursor-pointer shadow-sm"
                  >
                  ...
                  {makeupCardPeriod === 'custom' && (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="bg-white border border-[#d8d6d0] rounded-lg px-2 py-1 text-xs text-[#37352f] focus:outline-none focus:border-blue-500 transition-all shadow-sm"
                      />
                ...
                <button 
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-md text-xs font-black hover:bg-blue-700 transition-all shadow-md shrink-0"
                >
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 max-h-48 overflow-y-auto pr-0.5 custom-scrollbar-v">
                            {group.items.map((makeup) => {
                              ...
                              return (
                                <div key={makeup.id} className="py-2 border-b border-[#edece9] last:border-0 group/row space-y-1.5">
                                  <div className="flex items-center justify-between">
                                    <div className="flex flex-col min-w-0 pr-2">
                                      <div className="flex items-center gap-1.5 min-w-0">
                                        <span className="text-xs font-bold text-[#37352f] truncate">{makeup.student_name}</span>
                                        <span className="text-[8.5px] font-black px-1.5 py-0.2 bg-blue-50 text-blue-700 border border-blue-200 rounded shrink-0 shadow-sm">
                                          이번 달 {monthlyCount}회차
                                        </span>
                                        <span className="text-[8.5px] font-bold px-1.5 py-0.2 bg-purple-50 text-purple-700 border border-purple-200 rounded shrink-0 shadow-sm">
                                          담당: {teacherName}
                                        </span>
                                        {(() => {
                                          ...
                                          return (
                                            <span className="text-[8.5px] font-black px-1.5 py-0.2 bg-amber-50 text-amber-800 border border-amber-300 rounded shrink-0 flex items-center gap-0.5 shadow-sm">
                                              📅 {reasonText}
                                            </span>
                                          );
                                        })()}
                                      </div>
                                      <span className="text-[8.5px] font-black text-gray-400 uppercase mt-0.5">
                                      ...
                                      {editingMemoId === makeup.id ? (
                                        <div className="flex items-center gap-1 w-full">
                                          <input
                                            ...
                                            className="flex-1 bg-white border border-blue-400 rounded px-2 py-0.5 text-[10px] text-[#37352f] focus:outline-none shadow-sm"
                                            autoFocus
                                          />
                                          <button
                                            type="button"
                                            onClick={() => handleSaveMemo(makeup.id)}
                                            className="px-2 py-0.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-[9.5px] font-black shrink-0 shadow-sm"
                                          >
                                            저장
                                          </button>
                                          <button
                                            ...
                                            className="px-1.5 py-0.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded text-[9.5px] font-black shrink-0 border border-gray-200"
                                          >
                                            취소
                                          </button>
                                        </div>
                                        ...
                                        <div 
                                          ...
                                          className="flex items-center justify-between text-gray-500 hover:text-blue-600 cursor-pointer group/memo py-0.5 px-1 rounded hover:bg-black/5 transition-all"
                                          title="클릭하여 메모 수정"
                                        >
                                          <div className="flex items-center gap-1 min-w-0 pr-1">
                                            <FileText size={10} className="text-gray-400 shrink-0" />
                                            <span className={`truncate text-[9.5px] ${makeup.attendance_reason ? "text-blue-700 font-medium" : "text-gray-400 italic"}`}>
                                              {makeup.attendance_reason || '메모 작성...'}
                                            </span>
                                          </div>
                                          <Edit2 size={9} className="text-gray-400 group-hover/memo:text-blue-600 shrink-0 opacity-0 group-hover/memo:opacity-100 transition-opacity" />
                                        </div>
                                      )}
                                  </div>
                                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <select 
                      ...
                      className="w-full bg-white border border-[#edece9] rounded-lg px-3 py-2 text-xs text-[#37352f] focus:outline-none focus:border-blue-500 transition-all cursor-pointer shadow-sm"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-amber-700 uppercase tracking-widest flex items-center gap-1">
                      결석 원인 날짜 <span className="text-[8px] font-normal text-gray-400">(선택)</span>
                    </label>
                    <input
                      type="text"
                      value={makeupReason}
                      ...
                      className="w-full bg-white border border-amber-300 rounded-lg px-3 py-2 text-xs text-amber-900 placeholder-gray-400 focus:outline-none focus:border-amber-500 transition-all shadow-sm"
                    />
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-1.5 cursor-pointer select-none">
                        <input 
                          ...
                          className="w-3.5 h-3.5 rounded border border-gray-300 bg-white text-blue-600 focus:ring-0 focus:ring-offset-0 cursor-pointer accent-blue-600"
                        />
                        <span className="text-[10px] font-bold text-gray-700 hover:text-gray-900 transition-all">정규만</span>
                      </label>
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer select-none">
                        <input 
                          ...
                          className="w-3.5 h-3.5 rounded border border-gray-300 bg-white text-amber-600 focus:ring-0 focus:ring-offset-0 cursor-pointer accent-amber-600"
                        />
                        <span className="text-[10px] font-bold text-amber-700 hover:text-amber-900 transition-all">선택과목만</span>
                      </label>
                    </div>
                    ...
                    {(currentUser?.role === 'admin' || currentUser?.role === 'master') && (
                      <label className="flex items-center gap-1.5 cursor-pointer select-none">
                        <input 
                          ...
                          className="w-3.5 h-3.5 rounded border border-gray-300 bg-white text-blue-600 focus:ring-0 focus:ring-offset-0 cursor-pointer accent-blue-600"
                        />
                        <span className="text-[10px] font-black text-gray-500 hover:text-gray-900 transition-all uppercase tracking-wider">내 담당 학생만 보기</span>
                      </label>
                    )}
                  </div>
                          ...
                          return (
                            <div 
                              ...
                              className={`flex items-center justify-between px-3 py-1.5 rounded-md cursor-pointer text-xs font-bold transition-all ${
                                isSelected 
                                  ? (isSpecial ? 'bg-amber-50 text-amber-800 border border-amber-300 shadow-sm' : 'bg-blue-50 text-blue-650 border border-blue-200 shadow-sm') 
                                  : 'hover:bg-gray-100 text-gray-650'
                              }`}
                            >
                              <div className="flex items-center gap-1.5">
                                {isSpecial && (
                                  <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 border border-amber-300">
                                    {courseSubject}
                                  </span>
                                )}
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1.5">
                      {selectedStudentIds.map(itemKey => {
                        ...
                        return (
                          <div 
                            key={itemKey} 
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black shadow-sm border ${
                              isSpecial 
                          ...
                          >
                            ...
                            <button 
                              ...
                              className={isSpecial ? "text-amber-600 hover:text-amber-900 transition-colors" : "text-blue-500 hover:text-blue-300 transition-colors"}
                            >
                              <X size={10} />
                            </button>
                          </div>
...
// --- Sub-component: SuggestionHistoryView (Light Mode) ---
function SuggestionHistoryView({ tasks, toggleTask, deleteTask, isAdmin }: any) {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col overflow-hidden bg-[#f4f4f5] p-1 rounded-xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-[#e3e2e0] p-4 rounded-xl shadow-sm shrink-0">
        <div className="flex items-center gap-3">
