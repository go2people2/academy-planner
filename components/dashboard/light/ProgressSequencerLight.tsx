...
export default function ProgressSequencer({ students, masterTextbooks, initialStudentId, onSaveLegacy }: ProgressSequencerProps) {
  ...
  return (
    <div className="flex h-full bg-[#f4f4f5] overflow-hidden">
      {/* 1. 왼쪽: 학생 목록 */}
      <div className="w-64 border-r border-[#e3e2e0] flex flex-col bg-white">
        <div className="p-4 border-b border-[#edece9]">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-white border border-[#edece9] rounded-[4px] py-2 pl-9 pr-3 text-[11px] text-[#37352f] focus:outline-none focus:border-blue-500 transition-all font-bold placeholder-gray-300"
            />
          </div>
        </div>
          ...
          {filteredStudents.length > 0 ? (
            filteredStudents.map((s, idx) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-[4px] transition-all border group ${selectedStudentId === s.id ? 'bg-blue-50 text-blue-600 border-blue-200 shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-100 hover:text-[#37352f] border-transparent'}`}
              >
                <div className="flex flex-col items-start min-w-0">
                  <span className="text-[13px] font-black truncate w-full text-left">{s.name}</span>
                  <span className={`text-[8px] font-bold uppercase tracking-tighter ${selectedStudentId === s.id ? 'text-blue-600' : 'text-gray-400'}`}>{s.grade} · {s.course}</span>
                </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-[#f4f4f5]">
        {selectedStudent ? (
          <div className="flex-1 overflow-y-auto custom-scrollbar-v p-5 space-y-8">
            <div className="space-y-0.5 mb-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-[#37352f] uppercase tracking-tight">{selectedStudent.name} 학생 진도표</h2>
                  <p className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em]">학습 진행도를 확인합니다.</p>
                </div>
                {/* 💡 스마트 프로그레스 바 범례 추가 */}
                <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-full border border-[#e3e2e0] shadow-sm">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-emerald-500" />
                    <span className="text-[9px] font-black text-emerald-600 uppercase">오답완료</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-amber-500" />
                    <span className="text-[9px] font-black text-amber-600 uppercase">처음풀기</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-blue-600" />
                    <span className="text-[9px] font-black text-blue-600 uppercase">숙제부여</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400 font-bold text-[11px] uppercase tracking-wider">
    ...
    <div className={`space-y-2 transition-opacity ${isKeep ? 'opacity-70' : ''}`}>
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded flex items-center justify-center border transition-colors ${isKeep ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-blue-50 text-blue-600 border border-blue-200'}`}>
            <BookOpen size={14} />
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="space-y-0.5">
                    <h4 className={`text-[10px] font-black tracking-tight truncate w-24 ${isCompleted ? 'text-emerald-700' : 'text-gray-700'}`} title={u.unit}>{u.unit}</h4>
                  </div>
                  <div className="flex items-center gap-1.5">
                    {isCompleted ? (
                ...
                <div className="flex justify-between items-end mb-1.5">
                  <span className="text-[11px] font-black text-[#37352f] tabular-nums">{Math.round(progressRatio * 100)}%</span>
