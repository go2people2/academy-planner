    ...
    <div className="absolute inset-0 flex flex-col space-y-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          {/* 세그먼트 필터 버튼 (라이트 테마) */}
          <div className="flex items-center bg-[#f7f7f5] border border-[#e3e2e0] p-0.5 rounded-lg gap-1">
            <button
              type="button"
        ...
        <button 
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-md text-xs font-black hover:bg-blue-700 transition-all shadow-sm"
        >
      ...
      <div className="flex-1 overflow-y-auto pr-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 custom-scrollbar-v align-start content-start">
        {linkTasks.map((task) => {
          ...
          return (
            <motion.div 
              key={task.id}
              layout
              className={`group relative flex flex-col justify-between border border-[#e3e2e0] rounded-xl p-3 bg-white transition-all shadow-[0_1px_4px_rgba(15,15,15,0.08)] ${
                task.is_completed ? 'border-emerald-300 opacity-60 bg-gray-50/50' : 'hover:border-blue-300'
              }`}
            >
              <div className="space-y-1.5">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex flex-col min-w-0 pr-1">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      {task.isPrivate ? (
                        <span className="flex items-center gap-0.5 text-[8.5px] font-black px-1.5 py-0.2 bg-amber-50 text-amber-700 border border-amber-200 rounded shrink-0">
                          <Lock size={9} /> 개인
                        </span>
                      ) : (
                        <span className="flex items-center gap-0.5 text-[8.5px] font-black px-1.5 py-0.2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded shrink-0">
                          <Globe size={9} /> 공유
                        </span>
                      )}
                    </div>
                    <h4 className={`text-sm font-black leading-tight ${task.is_completed ? 'text-gray-400 line-through' : 'text-[#37352f]'}`}>
                      {task.title}
                    </h4>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button 
                      ...
                      className="w-5 h-5 rounded-full flex items-center justify-center border border-gray-200 text-gray-400 hover:border-blue-500 hover:text-blue-500 transition-all opacity-0 group-hover:opacity-100"
                      title="수정"
                    >
                      <Edit2 size={10} />
                    </button>
                ...
                <p className={`text-xs leading-relaxed ${task.is_completed ? 'text-gray-400' : 'text-[#37352f]/70'}`}>
                  {task.textContent || '링크에 대한 세부 설명이 없습니다.'}
                </p>
              </div>

              {/* Lower Section */}
              <div className="mt-3 pt-2.5 border-t border-[#edece9] flex flex-col gap-2">
                {/* Link button */}
                <button
                  ...
                  className={`w-full py-1.5 rounded-lg flex items-center justify-center gap-1.5 text-xs font-black transition-all ${
                    task.is_completed
                      ...
                      : 'bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-600 hover:text-white hover:border-indigo-500 active:scale-95 shadow-sm'
                  }`}
                >
                ...
                <div className="flex items-center justify-between text-[10px] font-bold text-gray-400">
                  ...
                  <div className="px-2 py-0.5 rounded-[4px] uppercase font-black bg-indigo-50 text-indigo-700 border border-indigo-200">
                    상시
                  </div>
                </div>
              </div>
        ...
        {linkTasks.length === 0 && (
          <div className="col-span-full h-64 border border-dashed border-[#e3e2e0] rounded-xl flex flex-col items-center justify-center text-gray-400 bg-white shadow-sm gap-2">
            <Sparkles size={24} className="text-gray-300" />
            <span className="text-xs font-bold text-gray-400">등록된 업무 링크가 없습니다. 새 링크를 등록해 보세요.</span>
          </div>
        )}
      </div>
        ...
        {isLinkModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-white border border-[#e3e2e0] w-full max-w-md rounded-xl p-6 shadow-2xl relative"
            >
              <button 
              ...
              <h3 className="text-base font-black text-[#37352f] uppercase tracking-wider mb-6 flex items-center gap-2">
                {editingLinkTask ? (
                  <>
                    <Edit2 size={18} className="text-blue-600" /> 업무 링크 수정
                  </>
                ) : (
                  <>
                    <Plus size={18} className="text-blue-600" /> 새 업무 링크 등록
                  </>
                )}
              </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-left">
                ...
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-2 p-1 bg-[#f7f7f5] border border-[#e3e2e0] rounded-lg">
                    <button
                      type="button"
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-[#edece9] rounded-lg px-3 py-2 text-xs text-[#37352f] placeholder-gray-300 focus:outline-none focus:border-blue-500 transition-all"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white border border-[#edece9] rounded-lg px-3 py-2 text-xs text-[#37352f] placeholder-gray-300 focus:outline-none focus:border-blue-500 transition-all resize-none"
                  />
                </div>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[11px] font-black transition-all ${
                            isSelected 
                              ...
                              : 'bg-white border-[#edece9] text-gray-500 hover:bg-gray-100 hover:text-gray-700'
                          }`}
                        >
                ...
                <div className="pt-2 flex justify-end gap-2">
                  ...
                  <button 
                    type="submit" 
                    disabled={isSaving}
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-black transition-all flex items-center gap-1.5 shadow-sm"
                  >
